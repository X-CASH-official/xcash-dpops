...
long long int add_block_to_blocks_found()
{
  ...
  // get the block count
  if (read_multiple_documents_all_fields_from_collection(DATABASE_NAME_DELEGATES,"blocks_found","",&database_data,1,document_count,1,"total_vote_count_number",0) == 0)
  {
...
int calculate_block_reward_for_each_delegate(long long int block_reward)
{
  ...
  char current_delegates_public_address[BUFFER_SIZE];
  ...
  for (count = 0; count < database_data.document_count; count++)
  {
    ...
    current_delegates_block_reward = (long long int)(((double)number / (double)total_votes) * (double)block_reward) | 0;
