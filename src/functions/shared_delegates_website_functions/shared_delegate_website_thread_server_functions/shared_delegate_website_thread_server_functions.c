...
long long int add_block_to_blocks_found(void)
{
  ...
  {
    // get the block count
    if (read_multiple_documents_all_fields_from_collection(shared_delegates_database_name,DATABASE_COLLECTION,"",&database_multiple_documents_fields,1,document_count,1,"total_vote_count_number",1) == 0)
    {
    ...
    if (current_UTC_date_and_time.tm_hour == shared_delegate_payment_time_hour && current_UTC_date_and_time.tm_min == shared_delegate_payment_time_minute)
    {
      ...
      for (count = 0; count < database_multiple_documents_fields.document_count; count++)
      {
        ...
        */
        ...
        if (read_document_field_from_collection(shared_delegates_database_name,"public_addresses",data,"inactivity_count",data3,1) == 0)
        {
