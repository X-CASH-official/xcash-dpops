...
int block_verifiers_synchronize_server_functions_test(void)
{
  ...
  if (sign_data(data_test) == 1 && send_and_receive_data_socket(result_test,sizeof(result_test),XCASH_DPOPS_delegates_IP_address,SEND_DATA_PORT,data_test,SEND_OR_RECEIVE_SOCKET_DATA_TIMEOUT_SETTINGS) == 1 && strncmp(result_test,RESERVE_BYTES_SYNC_CHECK_ALL_UPDATE_TEST_DATA,sizeof(RESERVE_BYTES_SYNC_CHECK_ALL_UPDATE_TEST_DATA)-1) == 0)
  {
