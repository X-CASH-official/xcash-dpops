...
int optimizations_functions_test(void)
{
  ...
  #define MAXIMUM_TIME_BLOCK_VERIFIERS_SEND_DATA_SOCKET 15
  #define MAXIMUM_TIME_SEND_AND_RECEIVE_DATA_SOCKET_THREAD 15
  ...
  #define MAXIMUM_TIME_VRF_DATA_VERIFY 15
  #define MAXIMUM_TIME_NETWORK_BLOCK_PART_1 15
  #define MAXIMUM_TIME_NETWORK_BLOCK_PART_2 20
  #define MAXIMUM_TIME_NETWORK_BLOCK_PART_3 15
  #define MAXIMUM_TIME_NETWORK_BLOCK_PART_4 15
  #define MAXIMUM_TIME_NETWORK_BLOCK_PART_5 15
  ...
  INITIALIZE_DATABASE_DATA(2);
  RESET_INVALID_RESERVE_PROOFS_DATA;
  for (count = 0; count < (MAXIMUM_INVALID_RESERVE_PROOFS / TOTAL_RESERVE_PROOFS_DATABASES) - 1; count++)
  {
    invalid_reserve_proofs.block_verifier_public_address[count] = (char*)calloc(XCASH_WALLET_LENGTH+1,sizeof(char));
    invalid_reserve_proofs.public_address_created_reserve_proof[count] = (char*)calloc(XCASH_WALLET_LENGTH+1,sizeof(char));
    invalid_reserve_proofs.public_address_voted_for[count] = (char*)calloc(XCASH_WALLET_LENGTH+1,sizeof(char));
  ...
  invalid_reserve_proofs.count = (int)((MAXIMUM_INVALID_RESERVE_PROOFS / TOTAL_RESERVE_PROOFS_DATABASES) - 1);
  start = time(NULL);
